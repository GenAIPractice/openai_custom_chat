...
openai.api_key = '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'
...
with personal_chatbot:
    ...
    if uploaded_file :
        #use tempfile because CSVLoader only accepts a file_path
            ...
            # data = data.encode()
            print(data)
            embeddings = OpenAIEmbeddings(api_key='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')
            vectorstore = FAISS.from_documents(data, embeddings)
            print("vectorstore ::: ", vectorstore.index.ntotal)
            print(vectorstore)
            chain = ConversationalRetrievalChain.from_llm(
            llm = ChatOpenAI(temperature=0.5, model_name='gpt-3.5-turbo', openai_api_key='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'),
            retriever=vectorstore.as_retriever(search_kwargs={"k": 1000}), max_tokens_limit=4097)
